...
const ThemeToggle = () => {
  ...
  return (
    <button 
      onClick={toggleTheme}
      className="p-2 rounded-full bg-surface border border-border-glass hover:bg-surface-highlight transition-all"
...
const SocialIcon = ({ href, label, path, colorClass }: { href: string; label: string; path: string; colorClass: string }) => (
  <a 
    ...
    aria-label={label}
    className={`w-10 h-10 flex items-center justify-center rounded-full bg-surface border border-border-glass text-text-muted transition-all duration-300 hover:scale-110 hover:text-white dark:hover:text-white ${colorClass}`}
  >
